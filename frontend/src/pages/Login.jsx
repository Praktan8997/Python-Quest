...
export const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'Failed to log in. Please check credentials.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="authkit-glass-card rounded-[16px] p-8 max-w-md w-full border border-[rgba(186,215,247,0.12)] bg-[rgba(186,214,247,0.03)] backdrop-blur-xl shadow-2xl relative z-10 space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-[#8AA4C4] font-sans">
            Log in to continue your Python Quest bootcamp
          </p>
        </div>
        ...
        <p className="text-center text-xs text-[#8AA4C4] font-sans">
          ...
          <Link to="/register" className="font-medium text-[#C8DCF5] hover:text-[#F0F6FF] transition-colors underline underline-offset-2">
            Sign Up
          </Link>
        </p>
      </div>
